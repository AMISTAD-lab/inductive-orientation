...
class Inductive_Generator:
  def __init__(self, mode, clf, classes, save_path, X_train, y_train, X_fixed = None, y_fixed=None, num_holdouts = 4, holdout_size = 5):
    """
  ...
  def get_LDM(self, X_test, num_datasets, num_repeat, proportion_of_dataset, data_generation_method, do_replace=False):
    ...
    if self.mode == "sparse":
      ...
      num_entries = int(proportion_of_dataset * len(self.X_train)) # size of each training subset (proportion_of_dataset=0.15 usually)

      def generateLDMHelper(dataset_idx):
        """Generates column vector in LDM with repeat_idx repeats"""
        ...
        def generatePf(repeat_idx):
          ...
          Pf = self.get_simplex(clf_copy, X_test) # note that each Pf is just a number
